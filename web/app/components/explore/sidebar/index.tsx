...
const SideBar: FC<IExploreSideBarProps> = ({
  ...
  const { show_discovery } = useAppContextSelector(s => s.intelligenceConfig)
  ...
  useEffect(() => {
    if (isDiscoverySelected && !show_discovery && installedApps.length > 0) router.push(`/explore/installed/${installedApps[0].id}`)
  }, [show_discovery, installedApps])

  const pinnedAppsCount = installedApps.filter(({ is_pinned }) => is_pinned).length
  return (
    <div className='w-fit sm:w-[216px] shrink-0 pt-6 px-4 border-r border-divider-burn cursor-pointer'>
      {show_discovery && (<div className={cn(isDiscoverySelected ? 'text-text-accent' : 'text-text-tertiary')}>
        <Link
          href='/explore/apps'
      ...
      {installedApps.length > 0 && (
        <div className={cn(show_discovery ? 'mt-10' : '')}>
